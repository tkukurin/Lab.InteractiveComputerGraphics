...
double intensity(glm::vec3 normal, glm::vec3 light)
{
	...
	glm::vec3 lightNormalized = glm::normalize(g_lightSource - light);
	double LN = glm::dot(normalNormalized, lightNormalized);

	return Ia * ka + Ii * ki * max(LN, 0.0d);
}

void color(glm::vec3 normal, glm::vec3 light) {
	double totalComponent = intensity(normal, light);
	glColor3ub(totalComponent, totalComponent, totalComponent);
}
...
void colorModelConstant(glm::vec3 v1, glm::vec3 v2, glm::vec3 v3, glm::vec3 colorComponents)
{
	...
	glBegin(GL_TRIANGLES);
	color(triangleNormal(v1,v2,v3), triangleCenter(v1, v2, v3));
	glVertex3f(v1.x, v1.y, v1.z);
	glVertex3f(v2.x, v2.y, v2.z);
	glVertex3f(v3.x, v3.y, v3.z);
}
...
void colorModelGouraud(glm::vec3 v1, glm::vec3 v2, glm::vec3 v3, glm::vec3 colorComponents)
{
	glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
	glBegin(GL_TRIANGLES);
	color(meanNormal(g_indexToNormals[indexof(v1, g_points)]), v1);
	glVertex3f(v1.x, v1.y, v1.z);

	color(meanNormal(g_indexToNormals[indexof(v2, g_points)]), v2);
	glVertex3f(v2.x, v2.y, v2.z);

	color(meanNormal(g_indexToNormals[indexof(v3, g_points)]), v3);
	glVertex3f(v3.x, v3.y, v3.z);
}
