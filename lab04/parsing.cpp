...
vector<glm::vec3> readVertices(ifstream &f)
{
	...
	while (f >> lineType) {
		...
		glm::vec3 loadedVector(x, y, z);
		data.push_back(loadedVector);
		updateLimits(loadedVector);
